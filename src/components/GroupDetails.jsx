        ...
        <div className="max-w-4xl mx-auto p-6 pt-20">
                            ...
                            {balances.map((balance) => {
                                ...
                                return (
                                    <tr key={balance.userId} className="border-b">
                                        ...
                                        <td className={`px-4 py-3 text-right font-medium ${balance.netBalance > 0
                                                ? 'text-green-600'
                                ...
                                {group.expenses.slice().reverse().map((expense) => {
                                    ...
                                    return (
                                        <tr key={expense._id} className="border-b">
                                            ...
                                            <td className="px-4 py-3">
                                                <span className={`px-2 py-1 rounded-full text-xs ${expense.isSettlement
            ...
            {showAddExpense && (
                <AddExpenseModal
                    ...
                    onExpenseAdded={async (newExpense) => {
                        ...
                        // 2) Re-fetch the group (with updated balances) from the server
                        try {
                            const response = await axios.get(`/api/groups/${groupId}`);
                            setGroup(response.data.group);
                            setBalances(response.data.balances);
                        } catch (err) {
                            console.error("Failed to refresh group after adding expense:", err);
...
const AddExpenseModal = ({ groupId, members, onClose, onExpenseAdded }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl p-6 mt-20 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-4">
                        ...
                        <select
                            ...
                            className="w-full px-3 cursor-pointer py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                        >
                    ...
                    <div className="flex justify-end">
                        <button
                            type="button"
                            onClick={onClose}
                            className="px-4 py-2 cursor-pointer mr-2 text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300"
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            className="px-4 py-2 cursor-pointer bg-green-600 text-white rounded-md hover:bg-green-700 disabled:bg-green-400 disabled:cursor-not-allowed"
                            disabled={loading}
                        >
